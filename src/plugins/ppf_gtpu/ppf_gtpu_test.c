...
static void vl_api_ppf_gtpu_add_del_tunnel_reply_t_handler
  (vl_api_ppf_gtpu_add_del_tunnel_reply_t * mp)
{
...
static void vl_api_ppf_gtpu_add_del_tunnel_v2_reply_t_handler
  (vl_api_ppf_gtpu_add_del_tunnel_v2_reply_t * mp)
{
  vat_main_t *vam = &vat_main;
  i32 retval = ntohl (mp->retval);
  if (vam->async_mode)
...
#define foreach_standard_reply_retval_handler   \
    _(sw_interface_set_ppf_gtpu_bypass_reply)

#define _(n)                                            \
    static void vl_api_##n##_t_handler                  \
    (vl_api_##n##_t * mp)                               \
    {                                                   \
        vat_main_t * vam = ppf_gtpu_test_main.vat_main;   \
        i32 retval = ntohl(mp->retval);                 \
        if (vam->async_mode) {                          \
            vam->async_errors += (retval < 0);          \
        } else {                                        \
...
#define foreach_vpe_api_reply_msg                               \
  _(SW_INTERFACE_SET_PPF_GTPU_BYPASS_REPLY, sw_interface_set_ppf_gtpu_bypass_reply) \
  _(PPF_GTPU_ADD_DEL_TUNNEL_REPLY, ppf_gtpu_add_del_tunnel_reply)               \
  _(PPF_GTPU_ADD_DEL_TUNNEL_V2_REPLY, ppf_gtpu_add_del_tunnel_v2_reply)               \
  _(PPF_GTPU_TUNNEL_DETAILS, ppf_gtpu_tunnel_details)
...
static int
api_sw_interface_set_ppf_gtpu_bypass (vat_main_t * vam)
{
  ...
  M (SW_INTERFACE_SET_PPF_GTPU_BYPASS, mp);
...
static uword unformat_ppf_gtpu_decap_next
...
static int
api_ppf_gtpu_add_del_tunnel_v2 (vat_main_t * vam)
{
...
static int
api_ppf_gtpu_add_del_tunnel (vat_main_t * vam)
{
  unformat_input_t *line_input = vam->input;
...
static void vl_api_ppf_gtpu_tunnel_details_t_handler
  (vl_api_ppf_gtpu_tunnel_details_t * mp)
{
  vat_main_t *vam = &vat_main;
...
static int
api_ppf_gtpu_tunnel_dump (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  vl_api_ppf_gtpu_tunnel_dump_t *mp;
  u32 sw_if_index;
  u8 sw_if_index_set = 0;
  int ret;
