/*
 * decap.c: ppf_gtpu tunnel decap packet processing
 *
...
static u8 *
format_ppf_gtpu_rx_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ppf_gtpu_rx_trace_t *t = va_arg (*args, ppf_gtpu_rx_trace_t *);

  if (t->tunnel_index != ~0)
    {
      s =
	format (s,
		"PPF_GTPU decap from ppf_gtpu_tunnel%d teid %u next %d error %d",
		t->tunnel_index, t->teid, t->next_index, t->error);
    }
  else
    {
      s =
	format (s, "PPF_GTPU decap error - tunnel for teid %u does not exist",
		t->teid);
    }
  return s;
}
...
always_inline u32
validate_ppf_gtpu_fib (vlib_buffer_t * b, ppf_gtpu_tunnel_t * t, u32 is_ip4)
{
		...
		vlib_frame_t * from_frame, u32 is_ip4)
{
  ...
  ppf_gtpu4_tunnel_key_t last_key4;
  ppf_gtpu6_tunnel_key_t last_key6;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  /* udp leaves current_data pointing at the ppf_gtpu header */
	  ppf_gtpu0 = vlib_buffer_get_current (b0);
	  ppf_gtpu1 = vlib_buffer_get_current (b1);
	  if (is_ip4)
	    {
	      vlib_buffer_advance
	  ...
	  if (PREDICT_FALSE
	      ((ppf_gtpu0->ver_flags & PPF_GTPU_VER_MASK) != PPF_GTPU_V1_VER))
	    {
	  ...
	  if (is_ip4)
		...
		tunnel_index0 = last_tunnel_index;
	      ...
	      /* Validate PPF_GTPU tunnel encap-fib index agaist packet */
	      if (PREDICT_FALSE (validate_ppf_gtpu_fib (b0, t0, is_ip4) == 0))
		{
		  error0 = PPF_GTPU_ERROR_NO_SUCH_TUNNEL;
		  next0 = PPF_GTPU_INPUT_NEXT_DROP;
		  goto trace0;
	...
	next0:
	  ...
	  if (PREDICT_FALSE
	      ((ppf_gtpu0->ver_flags & PPF_GTPU_E_S_PN_BIT) != 0))
	    {
	  ...
	  if (callline0->lbo_mode == PPF_LBO_MODE)
	    vnet_buffer (b0)->sw_if_index[VLIB_TX] =
	      vec_elt (ip4m->fib_index_by_sw_if_index,
		       callline0->sw_if_index);
	...
	trace0:
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      ppf_gtpu_rx_trace_t *tr
		= vlib_add_trace (vm, node, b0, sizeof (*tr));
	      tr->next_index = next0;
	      tr->error = error0;
	      tr->tunnel_index = tunnel_index0;
	      tr->teid = clib_net_to_host_u32 (ppf_gtpu0->teid);
	    }

	  if (PREDICT_FALSE
	      ((ppf_gtpu1->ver_flags & PPF_GTPU_VER_MASK) != PPF_GTPU_V1_VER))
	    {
	      error1 = PPF_GTPU_ERROR_BAD_VER;
	  ...
	  if (is_ip4)
		...
		tunnel_index1 = last_tunnel_index;
	      ...
	      /* Validate PPF_GTPU tunnel encap-fib index agaist packet */
	      if (PREDICT_FALSE (validate_ppf_gtpu_fib (b1, t1, is_ip4) == 0))
		{
		  error1 = PPF_GTPU_ERROR_NO_SUCH_TUNNEL;
		  next1 = PPF_GTPU_INPUT_NEXT_DROP;
		  goto trace1;
	...
	next1:
	  /* Manipulate ppf_gtpu header */
	  if (PREDICT_FALSE
	      ((ppf_gtpu1->ver_flags & PPF_GTPU_E_S_PN_BIT) != 0))
	    {
	    ...
	    {
	      ppf_gtpu_hdr_len1 = sizeof (ppf_gtpu_header_t) - 4;
	    }

	  /* Pop ppf_gtpu header */
	  vlib_buffer_advance (b1, ppf_gtpu_hdr_len1);
	  ...
	  vnet_buffer2 (b1)->ppf_du_metadata.tunnel_id[VLIB_RX_TUNNEL] =
	    tunnel_index1;
	...
	trace1:
	  ...
	  if (PREDICT_FALSE (b1->flags & VLIB_BUFFER_IS_TRACED))
		...
		= vlib_add_trace (vm, node, b1, sizeof (*tr));
	      ...
	      tr->teid = clib_net_to_host_u32 (ppf_gtpu1->teid);
	    }
  ...
  vlib_node_increment_counter (vm, is_ip4 ?
			       ppf_gtpu4_input_node.index :
			       ppf_gtpu6_input_node.index,
			       PPF_GTPU_ERROR_DECAPSULATED,
			       pkts_decapsulated);
...
static uword
ppf_gtpu4_input (vlib_main_t * vm,
		 vlib_node_runtime_t * node, vlib_frame_t * from_frame)
{
  return ppf_gtpu_input (vm, node, from_frame, /* is_ip4 */ 1);
}

static uword
ppf_gtpu6_input (vlib_main_t * vm,
		 vlib_node_runtime_t * node, vlib_frame_t * from_frame)
{
  return ppf_gtpu_input (vm, node, from_frame, /* is_ip4 */ 0);
}
...
VLIB_REGISTER_NODE (ppf_gtpu4_input_node) =
{
  .function = ppf_gtpu4_input,.name = "ppf_gtpu4-input",
    /* Takes a vector of packets. */
    .vector_size = sizeof (u32),.n_errors = PPF_GTPU_N_ERROR,.error_strings =
    ppf_gtpu_error_strings,.n_next_nodes = PPF_GTPU_INPUT_N_NEXT,.next_nodes =
  {
#define _(s,n) [PPF_GTPU_INPUT_NEXT_##s] = n,
    foreach_ppf_gtpu_input_next
#undef _
  }
  ,
//temp  .format_buffer = format_ppf_gtpu_header,
    .format_trace = format_ppf_gtpu_rx_trace,
    // $$$$ .unformat_buffer = unformat_ppf_gtpu_header,
};

VLIB_NODE_FUNCTION_MULTIARCH (ppf_gtpu4_input_node, ppf_gtpu4_input)
VLIB_REGISTER_NODE (ppf_gtpu6_input_node) =
{
  .function = ppf_gtpu6_input,.name = "ppf_gtpu6-input",
    /* Takes a vector of packets. */
    .vector_size = sizeof (u32),.n_errors = PPF_GTPU_N_ERROR,.error_strings =
    ppf_gtpu_error_strings,.n_next_nodes = PPF_GTPU_INPUT_N_NEXT,.next_nodes =
  {
...
VLIB_NODE_FUNCTION_MULTIARCH (ppf_gtpu6_input_node, ppf_gtpu6_input)
     typedef enum
     {
       ...
       IP_PPF_GTPU_BYPASS_NEXT_PPF_GTPU,
       IP_PPF_GTPU_BYPASS_N_NEXT,
     } ip_vxan_bypass_next_t;

     always_inline uword
       ip_ppf_gtpu_bypass_inline (vlib_main_t * vm,
				  vlib_node_runtime_t * node,
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (udp0->dst_port != clib_host_to_net_u16 (UDP_DST_PORT_GTPU))
	    goto exit0;		/* not PPF_GTPU packet */
	  ...
	  if (is_ip4)
	      ...
	      if (addr4.as_u32 != ip40->dst_address.as_u32)
		{
		  if (!hash_get (gtm->vtep4, ip40->dst_address.as_u32))
		    goto exit0;	/* no local VTEP for PPF_GTPU packet */
	  ...
	  next0 = error0 ?
	    IP_PPF_GTPU_BYPASS_NEXT_DROP : IP_PPF_GTPU_BYPASS_NEXT_PPF_GTPU;
	  b0->error = error0 ? error_node->errors[error0] : 0;

	  /* ppf_gtpu-input node expect current at PPF_GTPU header */
	  if (is_ip4)
	...
	exit0:
	  ...
	  if (udp1->dst_port != clib_host_to_net_u16 (UDP_DST_PORT_GTPU))
	    goto exit1;		/* not PPF_GTPU packet */
	  ...
	  if (is_ip4)
	      ...
	      if (addr4.as_u32 != ip41->dst_address.as_u32)
		{
		  if (!hash_get (gtm->vtep4, ip41->dst_address.as_u32))
		    goto exit1;	/* no local VTEP for PPF_GTPU packet */
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
		...
		{
		  if (!hash_get (gtm->vtep4, ip40->dst_address.as_u32))
		    goto exit;	/* no local VTEP for PPF_GTPU packet */
		  addr4 = ip40->dst_address;
		}
	    }
	  else
...
static uword
ip4_ppf_gtpu_bypass (vlib_main_t * vm,
		     vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  return ip_ppf_gtpu_bypass_inline (vm, node, frame, /* is_ip4 */ 1);
}

VLIB_REGISTER_NODE (ip4_ppf_gtpu_bypass_node) =
{
  .function = ip4_ppf_gtpu_bypass,.name = "ip4-ppf_gtpu-bypass",.vector_size =
    sizeof (u32),.n_next_nodes = IP_PPF_GTPU_BYPASS_N_NEXT,.next_nodes =
  {
  [IP_PPF_GTPU_BYPASS_NEXT_DROP] = "error-drop",
      [IP_PPF_GTPU_BYPASS_NEXT_PPF_GTPU] = "ppf_gtpu4-input",}
,.format_buffer = format_ip4_header,.format_trace =
    format_ip4_forward_next_trace,};

VLIB_NODE_FUNCTION_MULTIARCH (ip4_ppf_gtpu_bypass_node, ip4_ppf_gtpu_bypass)
/* Dummy init function to get us linked in. */
     clib_error_t *ip4_ppf_gtpu_bypass_init (vlib_main_t * vm)
{
  return 0;
}

VLIB_INIT_FUNCTION (ip4_ppf_gtpu_bypass_init);

static uword
ip6_ppf_gtpu_bypass (vlib_main_t * vm,
		     vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  return ip_ppf_gtpu_bypass_inline (vm, node, frame, /* is_ip4 */ 0);
}

VLIB_REGISTER_NODE (ip6_ppf_gtpu_bypass_node) =
{
  .function = ip6_ppf_gtpu_bypass,.name = "ip6-ppf_gtpu-bypass",.vector_size =
    sizeof (u32),.n_next_nodes = IP_PPF_GTPU_BYPASS_N_NEXT,.next_nodes =
  {
  [IP_PPF_GTPU_BYPASS_NEXT_DROP] = "error-drop",
      [IP_PPF_GTPU_BYPASS_NEXT_PPF_GTPU] = "ppf_gtpu6-input",}
,.format_buffer = format_ip6_header,.format_trace =
    format_ip6_forward_next_trace,};

VLIB_NODE_FUNCTION_MULTIARCH (ip6_ppf_gtpu_bypass_node, ip6_ppf_gtpu_bypass)
/* Dummy init function to get us linked in. */
     clib_error_t *ip6_ppf_gtpu_bypass_init (vlib_main_t * vm)
{
  return 0;
}

VLIB_INIT_FUNCTION (ip6_ppf_gtpu_bypass_init);
