...
bool
ppf_pdcp_eia0 (vlib_main_t * vm, vlib_buffer_t * b0,
	       void *security_parameters)
{
...
#define PDCP_USE_SSO_SNOW3G_ZUC 1

bool
ppf_pdcp_eia1_protect (vlib_main_t * vm, vlib_buffer_t * b0,
		       void *security_parameters)
{
  ...
  ppf_pdcp_security_param_t *sec_para =
    (ppf_pdcp_security_param_t *) security_parameters;
  CLIB_UNUSED (u8 iv[16]) =
  {
  0};
  #ifdef PDCP_USE_SSO_SNOW3G_ZUC
  sso_snow3g_key_schedule_t key_schedule;

  buf0 = vlib_buffer_get_current (b0);
  len = vlib_buffer_length_in_chain (vm, b0);
  
  ppf_pdcp_gen_iv(PDCP_SNOW3G_INTEGRITY, sec_para->count, sec_para->bearer, sec_para->dir, iv);
  sso_snow3g_init_key_sched(sec_para->pdcp_sess->crypto_key, &key_schedule);
  sso_snow3g_f9_1_buffer(&key_schedule, iv, (uint8_t *)buf0, len*8, vlib_buffer_put_uninit (b0, EIA_MAC_LEN));
  #else
  ...
  snow3g_protect (ctx, sec_para->pdcp_sess->crypto_key, sec_para->count,
		  sec_para->bearer, sec_para->dir, buf0, len,
		  vlib_buffer_put_uninit (b0, EIA_MAC_LEN));
...
bool
ppf_pdcp_eia1_validate (vlib_main_t * vm, vlib_buffer_t * b0,
			void *security_parameters)
{
  ...
  ppf_pdcp_gen_iv(PDCP_SNOW3G_INTEGRITY, sec_para->count, sec_para->bearer, sec_para->dir, iv);
  sso_snow3g_init_key_sched(sec_para->pdcp_sess->crypto_key, &key_schedule);
  sso_snow3g_f9_1_buffer(&key_schedule, iv, (uint8_t *)buf0, len*8, mact);
  #else
  ppf_pdcp_session_t *pdcp_sess = sec_para->pdcp_sess;
  snow3g_ctx_t *ctx = &(pdcp_sess->snow3g_ctx);
  ...
  len -= EIA_MAC_LEN;
  //TODO : use crypto key for intergity, align with wrong salt implemnt, need correct after salt FIX the bug
  snow3g_validate (ctx, sec_para->pdcp_sess->crypto_key, sec_para->count,
		   sec_para->bearer, sec_para->dir, buf0, len, mact);
  //snow3g_validate(ctx,sec_para->pdcp_sess->integrity_key,sec_para->count,sec_para->bearer,buf0,len,mact);
  #endif
  ret = (buf0[len + 0] == mact[0] && buf0[len + 1] == mact[1]
	 && buf0[len + 2] == mact[2] && buf0[len + 3] == mact[3]);
  //trim 4 octs of MAC
...
u32
ppf_pdcp_eea1_enc (u8 * in, u8 * out, u32 size, void *security_parameters)
{
  ppf_pdcp_security_param_t *sec_para =
    (ppf_pdcp_security_param_t *) security_parameters;
  CLIB_UNUSED (u8 iv[16]) =
  ...
  sso_snow3g_init_key_sched(sec_para->pdcp_sess->crypto_key, &key_schedule);
  sso_snow3g_f8_1_buffer(&key_schedule, iv, in, out, size);
  #else
  ppf_pdcp_session_t *pdcp_sess = sec_para->pdcp_sess;
...
u32
ppf_pdcp_eea1_dec (u8 * in, u8 * out, u32 size, void *security_parameters)
{
  ppf_pdcp_security_param_t *sec_para =
    (ppf_pdcp_security_param_t *) security_parameters;
  CLIB_UNUSED (u8 iv[16]) =
  ...
  ppf_pdcp_gen_iv(PDCP_SNOW3G_CIPHERING, sec_para->count, sec_para->bearer, sec_para->dir, iv);
  sso_snow3g_init_key_sched(sec_para->pdcp_sess->crypto_key, &key_schedule);
  sso_snow3g_f8_1_buffer(&key_schedule, (uint8_t *)iv, in, out, size);
  #else
  ppf_pdcp_session_t *pdcp_sess = sec_para->pdcp_sess;

  snow3g_ctx_t *ctx = &(pdcp_sess->snow3g_ctx);

  snow3g_decrypt (ctx, sec_para->pdcp_sess->crypto_key, sec_para->count,
		  sec_para->bearer, sec_para->dir, in, out, size);
  #endif
  return 0;
}
...
bool
ppf_pdcp_eia2_protect (vlib_main_t * vm, vlib_buffer_t * b0,
		       void *security_parameters)
{
  u8 mact[MAX_PDCP_KEY_LEN] = { 0 };
  u32 len;
...
bool
ppf_pdcp_eia2_validate (vlib_main_t * vm, vlib_buffer_t * b0,
			void *security_parameters)
{
  u8 mact[MAX_PDCP_KEY_LEN] = { 0 };
  u32 len;
...
u32
ppf_pdcp_eea2_enc (u8 * in, u8 * out, u32 size, void *security_parameters)
{
  ...
  EVP_CIPHER_CTX *ctx = pdcp_sess->down_sa.cypher_ctx;
#else
  EVP_CIPHER_CTX *ctx = &(pdcp_sess->down_sa.cypher_ctx);
#endif
...
u32
ppf_pdcp_eea2_dec (u8 * in, u8 * out, u32 size, void *security_parameters)
{
  u8 iv[MAX_PDCP_KEY_LEN] = { 0 };
  int out_len;
  ...
  ppf_pdcp_session_t *pdcp_sess = sec_para->pdcp_sess;
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  EVP_CIPHER_CTX *ctx = pdcp_sess->up_sa.cypher_ctx;
#else
  EVP_CIPHER_CTX *ctx = &(pdcp_sess->up_sa.cypher_ctx);
#endif
...
bool
ppf_pdcp_eia3_protect (vlib_main_t * vm, vlib_buffer_t * b0,
		       void *security_parameters)
{
    u32 len;
    u8 * buf0;
...
bool
ppf_pdcp_eia3_validate (vlib_main_t * vm, vlib_buffer_t * b0,
			void *security_parameters)
{
	u8 mact[MAX_PDCP_KEY_LEN] = {0};
...
ppf_pdcp_eea3_enc (u8 * in, u8 * out, u32 size, void *security_parameters)
{
    ...
    #ifdef PDCP_USE_SSO_SNOW3G_ZUC
    ppf_pdcp_gen_iv(PDCP_ZUC_CIPHERING, sec_para->count, sec_para->bearer, sec_para->dir, iv);
    sso_zuc_eea3_1_buffer(sec_para->pdcp_sess->crypto_key, iv, in, out, size);
    #else
	ppf_pdcp_session_t * pdcp_sess = sec_para->pdcp_sess;
	zuc_ctx_t *ctx = &(pdcp_sess->zuc_ctx);
...
u32
ppf_pdcp_create_session (u8 sn_length, u32 rx_count, u32 tx_count,
			 u32 in_flight_limit)
{
  ...
  pdcp_sess->rx_last_forwarded_sn =
    PPF_PDCP_SN_DEC (pdcp_sess->rx_next_expected_sn, sn_length) & max_sn;
  ...
  if (255 != sn_length)
    {
      ...
      clib_bitmap_alloc (pdcp_sess->rx_replay_bitmap,
			 pdcp_sess->replay_window << 1);
      clib_bitmap_zero (pdcp_sess->rx_replay_bitmap);

      vec_validate_init_empty (pdcp_sess->rx_reorder_buffers,
			       clib_min (ppm->rx_max_reorder_window,
					 pdcp_sess->replay_window) - 1,
...
u32
ppf_pdcp_session_update_as_security (ppf_pdcp_session_t * pdcp_sess,
				     ppf_pdcp_config_t * config)
{
  if (config->flags & INTEGRITY_KEY_VALID)
    clib_memcpy (pdcp_sess->integrity_key, config->integrity_key,
		 MAX_PDCP_KEY_LEN);
  ...
  if (config->flags & INTEGRITY_ALG_VALID)
	...
	case PDCP_EIA1:
	  ...
	  pdcp_sess->protect = &ppf_pdcp_eia1_protect;
	  pdcp_sess->validate = &ppf_pdcp_eia1_validate;
	  pdcp_sess->mac_length = 4;
	  break;

	case PDCP_EIA2:
	  pdcp_sess->security_algorithms |= PDCP_AES_INTEGRITY;
	  pdcp_sess->protect = &ppf_pdcp_eia2_protect;
	  pdcp_sess->validate = &ppf_pdcp_eia2_validate;
	  pdcp_sess->mac_length = 4;
	  pdcp_sess->up_sa.integrity_ctx = CMAC_CTX_new ();
	  pdcp_sess->down_sa.integrity_ctx = CMAC_CTX_new ();
	  CMAC_Init (pdcp_sess->up_sa.integrity_ctx, pdcp_sess->crypto_key,
		     16, EVP_aes_128_cbc (), NULL);
	  CMAC_Init (pdcp_sess->down_sa.integrity_ctx, pdcp_sess->crypto_key,
		     16, EVP_aes_128_cbc (), NULL);
	  break;

	case PDCP_EIA3:
	  pdcp_sess->security_algorithms |= PDCP_ZUC_INTEGRITY;
	  pdcp_sess->protect = &ppf_pdcp_eia3_protect;
	  pdcp_sess->validate = &ppf_pdcp_eia3_validate;
	  pdcp_sess->mac_length = 4;
	  break;
  ...
  if (config->flags & CRYPTO_ALG_VALID)
	...
	case PDCP_EEA1:
	  pdcp_sess->encrypt = &ppf_pdcp_eea1_enc;
	  pdcp_sess->decrypt = &ppf_pdcp_eea1_dec;
	  pdcp_sess->security_algorithms |= PDCP_SNOW3G_CIPHERING;
	  break;

	case PDCP_EEA2:
	  pdcp_sess->encrypt = &ppf_pdcp_eea2_enc;
	  pdcp_sess->decrypt = &ppf_pdcp_eea2_dec;
	  ...
	  EVP_CipherInit_ex (pdcp_sess->up_sa.cypher_ctx, EVP_aes_128_ctr (),
			     NULL, pdcp_sess->crypto_key, NULL, 0);
	  EVP_CipherInit_ex (pdcp_sess->down_sa.cypher_ctx,
			     EVP_aes_128_ctr (), NULL, pdcp_sess->crypto_key,
			     NULL, 1);

#else
	  EVP_CIPHER_CTX_init (&(pdcp_sess->up_sa.cypher_ctx));
	  EVP_CIPHER_CTX_init (&(pdcp_sess->down_sa.cypher_ctx));
	  EVP_CipherInit_ex (&(pdcp_sess->up_sa.cypher_ctx),
			     EVP_aes_128_ctr (), NULL, pdcp_sess->crypto_key,
			     NULL, 0);
	  EVP_CipherInit_ex (&(pdcp_sess->down_sa.cypher_ctx),
			     EVP_aes_128_ctr (), NULL, pdcp_sess->crypto_key,
	...
	case PDCP_EEA3:
	  pdcp_sess->encrypt = &ppf_pdcp_eea3_enc;
	  pdcp_sess->decrypt = &ppf_pdcp_eea3_dec;
  ...
  if (pdcp_sess)
    {
      ...
      {
	if (VEC_ELT_NE
	    (pdcp_sess->rx_reorder_buffers, i, INVALID_BUFFER_INDEX))
	  {
	    vlib_buffer_free_one (&vlib_global_main,
				  vec_elt (pdcp_sess->rx_reorder_buffers, i));
	  }
      }
...
format_ppf_pdcp_session (u8 * s, va_list * va)
{
  ...
  s =
    format (s,
	    "sn-length %d, header-length %d, mac-length %d, in-flight-limit %lx\n",
	    pdcp_session->sn_length, pdcp_session->header_length,
	    pdcp_session->mac_length, pdcp_session->in_flight_limit);

  s = format (s, "tx-hfn %u, tx-next %u\n",
	      pdcp_session->tx_hfn, pdcp_session->tx_next_sn);
  ...
  if (verbose > 1)
    {
      s = format (s, "replay-window %u, rx-replay-bitmap (%u bits)\n %U\n",
		  ...
		  clib_bitmap_bytes (pdcp_session->rx_replay_bitmap) *
		  BITS (u8), format_bitmap_hex,
		  pdcp_session->rx_replay_bitmap);
      s =
	format (s, "reorder-window %u, details\n%U\n",
		vec_len (pdcp_session->rx_reorder_buffers), format_vec32_adv,
		pdcp_session->rx_reorder_buffers, "%d", 8);
    }

  s = format (s, "integrity-alg %U, intergity-key %U\n",
	      format_pdcp_security_alg,
	      pdcp_session->security_algorithms & 0xf0, format_hex_bytes,
	      pdcp_session->integrity_key, MAX_PDCP_KEY_LEN);

  s = format (s, "crypto-alg %U, crypto-key %U\n",
	      format_pdcp_security_alg,
	      pdcp_session->security_algorithms & 0x0f, format_hex_bytes,
	      pdcp_session->crypto_key, MAX_PDCP_KEY_LEN);
				     ...
				     vlib_cli_command_t * cmd)
{
  ...
  u8 is_add = 1;
  u32 call_id = ~0;
  u32 sn_length = ~0, init_ul_count = 0, init_dl_count = 0, in_flight_limit =
    0;
  u32 alg;
  u8 *key = 0;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	if (unformat
	    (line_input, "crypto-alg %U", unformat_pdcp_crypto_alg, &alg))
	{
	...
	if (unformat
	    (line_input, "integ-alg %U", unformat_pdcp_integ_alg, &alg))
	{
  ...
  if (call_id >= ppf_main.max_capacity)
    {
      error =
	clib_error_return (0, "error call-id %u, should be in range[0, %u)",
			   call_id, ppf_main.max_capacity);
      goto done;
    }
  ...
  if (is_add == 1)
    {
      sess_id =
	ppf_pdcp_create_session (sn_length, init_ul_count, init_dl_count,
				 in_flight_limit);
      if (~0 != sess_id)
	{
	...
	{
	  error =
	    clib_error_return (0, "create session fail, error sn-length %u",
			       sn_length);
	  goto done;
	}
      ...
      vlib_cli_output (vm, "[PDCP session created], details: %U",
		       format_ppf_pdcp_session,
		       pool_elt_at_index (ppf_pdcp_main.sessions, sess_id),
		       2);
    }
  else
    {				/* del */
	...
	{
	  ppf_pdcp_clear_session (pool_elt_at_index
				  (ppf_pdcp_main.sessions,
	...
	{
	  error =
	    clib_error_return (0, "delete session fail, error call-id %u",
			       call_id);
	  goto done;
	}
    }
				 ...
				 vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "PPF PDCP reorder is set to %s, window %d\n",
		   ...
		   ppf_pdcp_main.rx_max_reorder_window);
		...
		unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  switch (op)
      ...
      while (loop)
	{
	  ...
	  vlib_cli_output (vm,
			   "loop %u - window [%u, %u], Bitmap: %u bits\n %U",
			   loop, bit + 1, bit + test_window,
			   BITMAP_LEN (test_bitmap) *
			   BITMAP_WORD_BITS (test_bitmap), format_bitmap_hex,
			   test_bitmap);
...
ppf_pdcp_init (vlib_main_t * vm)
{
  ...
  if (ppf_main.max_capacity)
    {
      pool_init_aligned (ppm->sessions, ppf_main.max_capacity,
