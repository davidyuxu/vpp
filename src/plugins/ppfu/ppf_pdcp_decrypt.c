...
u8 *
format_ppf_pdcp_decrypt_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "PDCP_DECRYPT: tunnel_index %d, call_id %d (type %U, ue_bearer %x)\nsn %u, hfn %u, integrity_status %d, error %s, next_index %d",
	    t->tunnel_index, t->call_id, format_ppf_call_type, t->call_type,
	    t->ue_bearer_id, t->sn, t->hfn, t->integrity_status,
	    ppf_pdcp_decrypt_error_strings[t->error], t->next_index);
			 ...
			 vlib_frame_t * frame, u32 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 8 && n_left_to_next >= 4)
	{
	  ...
	  u32 count_last_fwd0 = 0, count_last_fwd1 = 0, count_last_fwd2 =
	    0, count_last_fwd3 = 0;
	  ...
	  /* Prefetch next iteration. */
	  {
	    ...
	    CLIB_PREFETCH (p4->data,
			   clib_min (p4->current_data + p4->current_length,
				     4 * CLIB_CACHE_LINE_BYTES), STORE);
	    CLIB_PREFETCH (p5->data,
			   clib_min (p5->current_data + p5->current_length,
				     4 * CLIB_CACHE_LINE_BYTES), STORE);
	    CLIB_PREFETCH (p6->data,
			   clib_min (p6->current_data + p6->current_length,
				     4 * CLIB_CACHE_LINE_BYTES), STORE);
	    CLIB_PREFETCH (p7->data,
			   clib_min (p7->current_data + p7->current_length,
				     4 * CLIB_CACHE_LINE_BYTES), STORE);
	  }
	  ...
	  /* Find rx tunnel */
	  if (PREDICT_FALSE
	      (pool_is_free_index (gtm->tunnels, tunnel_index0)))
	    {
	  ...
	  if (PREDICT_FALSE (sn0 != pdcp0->rx_next_expected_sn))
		...
		hfn0 = PPF_PDCP_HFN_DEC (pdcp0->rx_hfn, pdcp0->sn_length);
	      ...
	      count_last_fwd0 =
		PPF_PDCP_COUNT (pdcp0->rx_hfn, pdcp0->rx_last_forwarded_sn,
				pdcp0->sn_length);
	      if (PREDICT_FALSE
		  ((count0 <= count_last_fwd0)
		   || (BITMAP_ON (pdcp0->rx_replay_bitmap, count0))))
		{
	  ...
	  if (pdcp0->mac_length)
		...
		{
		  vnet_buffer2 (b0)->ppf_du_metadata.pdcp.integrity_status =
		    2;
	...
	trace0:
	  ...
	  /* Find rx tunnel */
	  if (PREDICT_FALSE
	      (pool_is_free_index (gtm->tunnels, tunnel_index1)))
	    {
	  ...
	  if (PREDICT_FALSE (sn1 != pdcp1->rx_next_expected_sn))
		...
		hfn1 = PPF_PDCP_HFN_DEC (pdcp1->rx_hfn, pdcp1->sn_length);
	      ...
	      count_last_fwd1 =
		PPF_PDCP_COUNT (pdcp1->rx_hfn, pdcp1->rx_last_forwarded_sn,
				pdcp1->sn_length);
	      if (PREDICT_FALSE
		  ((count1 <= count_last_fwd1)
		   || (BITMAP_ON (pdcp1->rx_replay_bitmap, count1))))
		{
	  ...
	  if (pdcp1->mac_length)
		...
		{
		  vnet_buffer2 (b1)->ppf_du_metadata.pdcp.integrity_status =
		    2;
	...
	trace1:
	  ...
	  /* Get tunnel index from buffer */
	  tunnel_index2 =
	    vnet_buffer2 (b2)->ppf_du_metadata.tunnel_id[VLIB_RX_TUNNEL];

	  /* Find rx tunnel */
	  if (PREDICT_FALSE
	      (pool_is_free_index (gtm->tunnels, tunnel_index2)))
	    {
	  ...
	  if (PREDICT_FALSE (sn2 != pdcp2->rx_next_expected_sn))
		...
		hfn2 = PPF_PDCP_HFN_DEC (pdcp2->rx_hfn, pdcp2->sn_length);
	      ...
	      count_last_fwd2 =
		PPF_PDCP_COUNT (pdcp2->rx_hfn, pdcp2->rx_last_forwarded_sn,
				pdcp2->sn_length);
	      if (PREDICT_FALSE
		  ((count2 <= count_last_fwd2)
		   || (BITMAP_ON (pdcp2->rx_replay_bitmap, count2))))
		{
	  ...
	  if (pdcp2->mac_length)
		...
		{
		  vnet_buffer2 (b2)->ppf_du_metadata.pdcp.integrity_status =
		    2;
	...
	trace2:
	  ...
	  /* Get tunnel index from buffer */
	  tunnel_index3 =
	    vnet_buffer2 (b3)->ppf_du_metadata.tunnel_id[VLIB_RX_TUNNEL];

	  /* Find rx tunnel */
	  if (PREDICT_FALSE
	      (pool_is_free_index (gtm->tunnels, tunnel_index3)))
	    {
	  ...
	  if (PREDICT_FALSE (sn3 != pdcp3->rx_next_expected_sn))
		...
		hfn3 = PPF_PDCP_HFN_DEC (pdcp3->rx_hfn, pdcp3->sn_length);
	      ...
	      count_last_fwd3 =
		PPF_PDCP_COUNT (pdcp3->rx_hfn, pdcp3->rx_last_forwarded_sn,
				pdcp3->sn_length);
	      if (PREDICT_FALSE
		  ((count3 <= count_last_fwd3)
		   || (BITMAP_ON (pdcp3->rx_replay_bitmap, count3))))
		{
	  ...
	  if (pdcp3->mac_length)
		...
		{
		  vnet_buffer2 (b3)->ppf_du_metadata.pdcp.integrity_status =
		    2;
...
{
  .function = ppf_pdcp_decrypt,.name = "ppf_pdcp_decrypt",.vector_size =
    sizeof (u32),.format_trace = format_ppf_pdcp_decrypt_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_pdcp_decrypt_error_strings),.error_strings =
    ppf_pdcp_decrypt_error_strings,.n_next_nodes =
    PPF_PDCP_DECRYPT_N_NEXT,.next_nodes =
  {
...
u8 *
format_ppf_pdcp_reorder_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ...
  s =
    format (s,
	    "PDCP_REORDER: tunnel_index %d, call_id %d (type %U)\nsn %u, count %u, error %s, next_index %d",
	    t->tunnel_index, t->call_id, format_ppf_call_type, t->call_type,
	    t->sn, t->count, ppf_pdcp_reorder_error_strings[t->error],
	    t->next_index);
			 ...
			 vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  pdcp0 = pool_elt_at_index (ppm->sessions, c0->pdcp.session_id);
	  if (PREDICT_TRUE
	      (1 == vnet_buffer2 (b0)->ppf_du_metadata.pdcp.bypass_reorder))
	    {
	      goto next00;
	    }
	  ...
	  if (PREDICT_FALSE (sn0 != pdcp0->rx_next_expected_sn))
		...
		PPF_PDCP_COUNT (pdcp0->rx_hfn, pdcp0->rx_last_forwarded_sn,
	      ...
	      if (PREDICT_FALSE
		  (count0 - count_last_fwd0 >
		   vec_len (pdcp0->rx_reorder_buffers)))
		{
	      ...
	      if (VEC_ELT_NE
		  (pdcp0->rx_reorder_buffers, count0, INVALID_BUFFER_INDEX))
		{
	...
	next00:
	  ...
	  pdcp0->rx_next_expected_sn =
	    PPF_PDCP_SN_INC (sn0, pdcp0->sn_length);
	  ...
	  if (pm->handoff_enable)
		...
		next0 = PPF_PDCP_REORDER_NEXT_PPF_SRB_NB_TX;
	      ...
	      if (PREDICT_FALSE
		  ((c0->lbo_mode == PPF_LBO_MODE)
		   && (c0->call_type == PPF_DRB_CALL)))
	...
	trace00:
	  ...
	  while (VEC_ELT_NE
		 (pdcp0->rx_reorder_buffers, count0 + 1,
		  INVALID_BUFFER_INDEX))
	    {
	      next0 = next_index;
	      while (VEC_ELT_NE
		     (pdcp0->rx_reorder_buffers, count0 + 1,
		      INVALID_BUFFER_INDEX) && (n_left_to_next > 0))
		{
		  bi0 = VEC_AT (pdcp0->rx_reorder_buffers, count0 + 1);
		  VEC_SET (pdcp0->rx_reorder_buffers, count0 + 1,
			   INVALID_BUFFER_INDEX);
...
{
  .function = ppf_pdcp_reorder_inline,.name =
    "ppf_pdcp_reorder",.vector_size = sizeof (u32),.format_trace =
    format_ppf_pdcp_reorder_trace,.type = VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_pdcp_reorder_error_strings),.error_strings =
    ppf_pdcp_reorder_error_strings,.n_next_nodes =
    PPF_PDCP_REORDER_N_NEXT,.next_nodes =
  {
