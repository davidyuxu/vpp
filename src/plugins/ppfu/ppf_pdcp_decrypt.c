...
u8 * format_ppf_pdcp_decrypt_trace  (u8 * s, va_list * args)
{
  ...
  s = format (s, "PDCP_DECRYPT: tunnel_index %d, call_id %d (type %d, ue_bearer %x)\nsn %u, hfn %u, integrity_status %d, error %d",
		  t->tunnel_index, t->call_id, t->call_type, t->ue_bearer_id,
		  t->sn, t->hfn, t->integrity_status, t->error);
  return s;
}
	...
	while (n_left_from >= 12 && n_left_to_next >= 4)
	  {
	    ...
	    pdcp0->decrypt (buf0 + pdcp0->header_length,
            ...
            /* Validate */
	    pdcp0->validate (buf0, xmaci, len0 - pdcp0->mac_length, NULL);
	    if (pdcp0->mac_length) {
	    ...
	    pdcp1->decrypt (buf1 + pdcp1->header_length,
            ...
            /* Validate */
	    pdcp1->validate (buf1, xmaci, len1 - pdcp1->mac_length, NULL);
	    if (pdcp1->mac_length) {
	    ...
	    pdcp2->decrypt (buf2 + pdcp2->header_length,
            ...
            /* Validate */
	    pdcp2->validate (buf2, xmaci, len2 - pdcp2->mac_length, NULL);
	    if (pdcp2->mac_length) {
	    ...
	    pdcp3->decrypt (buf3 + pdcp3->header_length,
            ...
            /* Validate */
	    pdcp3->validate (buf3, xmaci, len3 - pdcp3->mac_length, NULL);
	    if (pdcp3->mac_length) {
