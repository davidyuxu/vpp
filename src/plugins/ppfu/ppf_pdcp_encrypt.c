...
u8 *
format_ppf_pdcp_encrypt_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "PDCP_ENCRYPT: tunnel_index %d, call_id %d (type %U, ue_bearer %x)\nsn %u, count %u, error %s, next_index %d",
	    t->tunnel_index, t->call_id, format_ppf_call_type, t->call_type,
	    t->ue_bearer_id, t->sn, t->count,
	    ppf_pdcp_encrypt_error_strings[t->error], t->next_index);
			 ...
			 vlib_frame_t * frame, u32 is_ip4)
{
  ...
  if (pm->handoff_enable)
    next_index = PPF_PDCP_ENCRYPT_NEXT_PPF_TX_HANDOFF;
	  ...
	  if (PREDICT_TRUE (PPF_DRB_CALL == c0->call_type))
	    {
	      ...
	      count0 =
		PPF_PDCP_COUNT (pdcp0->tx_hfn, pdcp0->tx_next_sn,
				pdcp0->sn_length);
	      PPF_PDCP_COUNT_INC (pdcp0->tx_hfn, pdcp0->tx_next_sn,
				  pdcp0->sn_length);
	    }
	  else
	    {
	  ...
	  if (PREDICT_TRUE (PPF_DRB_CALL == c1->call_type))
	    {
	      ...
	      count1 =
		PPF_PDCP_COUNT (pdcp1->tx_hfn, pdcp1->tx_next_sn,
				pdcp1->sn_length);
	      PPF_PDCP_COUNT_INC (pdcp1->tx_hfn, pdcp1->tx_next_sn,
				  pdcp1->sn_length);
	    }
	  else
	    {
	  ...
	  if (PREDICT_TRUE (PPF_DRB_CALL == c2->call_type))
	    {
	      ...
	      count2 =
		PPF_PDCP_COUNT (pdcp2->tx_hfn, pdcp2->tx_next_sn,
				pdcp2->sn_length);
	      PPF_PDCP_COUNT_INC (pdcp2->tx_hfn, pdcp2->tx_next_sn,
				  pdcp2->sn_length);
	    }
	  else
	    {
	  ...
	  if (PREDICT_TRUE (PPF_DRB_CALL == c3->call_type))
	    {
	      ...
	      count3 =
		PPF_PDCP_COUNT (pdcp3->tx_hfn, pdcp3->tx_next_sn,
				pdcp3->sn_length);
	      PPF_PDCP_COUNT_INC (pdcp3->tx_hfn, pdcp3->tx_next_sn,
				  pdcp3->sn_length);
	    }
	  else
	    {
...
{
  .function = ppf_pdcp_encrypt,.name = "ppf_pdcp_encrypt",.vector_size =
    sizeof (u32),.format_trace = format_ppf_pdcp_encrypt_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_pdcp_encrypt_error_strings),.error_strings =
    ppf_pdcp_encrypt_error_strings,.n_next_nodes =
    PPF_PDCP_ENCRYPT_N_NEXT,.next_nodes =
  {
