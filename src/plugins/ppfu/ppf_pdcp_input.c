...
u8 *
format_ppf_pdcp_input_trace (u8 * s, va_list * args)
{
		       ...
		       vlib_frame_t * frame, u32 is_ip4)
{
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_PDCP_INPUT_ERROR_GOOD, pkts_processed);
...
{
  .function = ppf_pdcp_input,.name = "ppf_pdcp_input",.vector_size =
    sizeof (u32),.format_trace = format_ppf_pdcp_input_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_pdcp_input_error_strings),.error_strings =
    ppf_pdcp_input_error_strings,.n_next_nodes =
    PPF_PDCP_INPUT_N_NEXT,.next_nodes =
  {
