...
u8 *
format_ppf_sb_path_lb_trace (u8 * s, va_list * args)
{
		       ...
		       vlib_frame_t * frame, u32 is_ip4)
{
  ...
  u32 *buffers_duplicated =
    pm->buffers_duplicated_per_thread[vlib_get_thread_index ()];
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 8 && n_left_to_next >= 4)
	{
	  ...
	  u32 tunnel_id0 = ~0, tunnel_id1 = ~0, tunnel_id2 = ~0, tunnel_id3 =
	    ~0;
	  ...
	  /* default as DRB */
	  {
		...
		tunnel_id0 =
		  callline0->rb.
		  drb.sb_tunnel[PPF_SB_VALID_PATH
				(callline0->sb_multi_path)].tunnel_id;
	  ...
	  if (PREDICT_FALSE (callline0->call_type == PPF_SRB_CALL))
		...
		{
		  tunnel_id0 =
		    callline0->rb.srb.sb_tunnel[sb_ids0[0]].tunnel_id;
		  vnet_buffer2 (b0)->
		    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] = tunnel_id0;
		}
	      ...
	      else
		{
		  ...
		  if (sb_num0 == 1)
		    {
		      tunnel_id0 =
			callline0->rb.srb.sb_tunnel[PPF_SB_VALID_PATH
						    (callline0->
						     sb_multi_path)].
			tunnel_id;
		      vnet_buffer2 (b0)->
			ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] =
			tunnel_id0;
		    }
		  else
		    {
	  ...
	  if (PREDICT_FALSE (sb_num0 > 1))
	      ...
	      for (id = 0; id < MAX_SB_PER_CALL; id++)
		{
		  ...
		  if ((INVALID_TUNNEL_ID == tunnel_id0)
		      ||
		      (PPF_SB_PATH_GET_VALID
		       (callline0->sb_multi_path, sb_ids0[id]) == 0))
		    continue;
		  ...
		  if (1 == duplicate)
		  ...
		  else if (duplicate > 1)
		    {
		      ...
		      if (c0)
			{
			  clib_memcpy (c0->opaque2, b0->opaque2,
				       sizeof (b0->opaque2));
			  vnet_buffer2 (c0)->
			    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] =
			    tunnel_id0;

			  vec_add1 (buffers_duplicated,
	...
	trace0:
	  ...
	  /* default as DRB */
	  {
		...
		tunnel_id1 =
		  callline1->rb.
		  drb.sb_tunnel[PPF_SB_VALID_PATH
				(callline1->sb_multi_path)].tunnel_id;
	  ...
	  if (PREDICT_FALSE (callline1->call_type == PPF_SRB_CALL))
		...
		{
		  tunnel_id1 =
		    callline1->rb.srb.sb_tunnel[sb_ids1[0]].tunnel_id;
		  vnet_buffer2 (b1)->
		    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] = tunnel_id1;
		}
	      ...
	      else
		{
		  ...
		  if (sb_num1 == 1)
		    {
		      tunnel_id1 =
			callline1->rb.srb.sb_tunnel[PPF_SB_VALID_PATH
						    (callline1->
						     sb_multi_path)].
			tunnel_id;
		      vnet_buffer2 (b1)->
	  ...
	  if (PREDICT_FALSE (sb_num1 > 1))
	      ...
	      for (id = 0; id < MAX_SB_PER_CALL; id++)
		{
		  ...
		  if ((INVALID_TUNNEL_ID == tunnel_id1)
		      ||
		      (PPF_SB_PATH_GET_VALID
		       (callline1->sb_multi_path, sb_ids1[id]) == 0))
		    continue;
		  ...
		  if (1 == duplicate)
		  ...
		  else if (duplicate > 1)
		    {
		      ...
		      if (c1)
			{
			  clib_memcpy (c1->opaque2, b1->opaque2,
				       sizeof (b1->opaque2));
			  vnet_buffer2 (c1)->
			    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] =
			    tunnel_id1;
	...
	trace1:
	  ...
	  /* default as DRB */
	  {
		...
		tunnel_id2 =
		  callline2->rb.
		  drb.sb_tunnel[PPF_SB_VALID_PATH
				(callline2->sb_multi_path)].tunnel_id;
	  ...
	  if (PREDICT_FALSE (callline2->call_type == PPF_SRB_CALL))
		...
		{
		  tunnel_id2 =
		    callline2->rb.srb.sb_tunnel[sb_ids2[0]].tunnel_id;
		  vnet_buffer2 (b2)->
		    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] = tunnel_id2;
		}
	      ...
	      else
		{
		  ...
		  if (sb_num2 == 1)
		    {
		      tunnel_id2 =
			callline2->rb.srb.sb_tunnel[PPF_SB_VALID_PATH
						    (callline2->
						     sb_multi_path)].
			tunnel_id;
		      vnet_buffer2 (b2)->
	  ...
	  if (PREDICT_FALSE (sb_num2 > 1))
	      ...
	      for (id = 0; id < MAX_SB_PER_CALL; id++)
		{
		  ...
		  if ((INVALID_TUNNEL_ID == tunnel_id2)
		      ||
		      (PPF_SB_PATH_GET_VALID
		       (callline2->sb_multi_path, sb_ids2[id]) == 0))
		    continue;
		  ...
		  if (1 == duplicate)
		  ...
		  else if (duplicate > 1)
		    {
		      ...
		      if (c2)
			{
			  clib_memcpy (c2->opaque2, b2->opaque2,
				       sizeof (b2->opaque2));
			  vnet_buffer2 (c2)->
			    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] =
			    tunnel_id2;
	...
	trace2:
	  ...
	  /* default as DRB */
	  {
		...
		tunnel_id3 =
		  callline3->rb.
		  drb.sb_tunnel[PPF_SB_VALID_PATH
				(callline3->sb_multi_path)].tunnel_id;
	  ...
	  if (PREDICT_FALSE (callline3->call_type == PPF_SRB_CALL))
		...
		{
		  tunnel_id3 =
		    callline3->rb.srb.sb_tunnel[sb_ids3[0]].tunnel_id;
		  vnet_buffer2 (b3)->
		    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] = tunnel_id3;
		}
	      ...
	      else
		{
		  ...
		  if (sb_num3 == 1)
		    {
		      tunnel_id3 =
			callline3->rb.srb.sb_tunnel[PPF_SB_VALID_PATH
						    (callline3->
						     sb_multi_path)].
			tunnel_id;
		      vnet_buffer2 (b3)->
	  ...
	  if (PREDICT_FALSE (sb_num3 > 1))
	      ...
	      for (id = 0; id < MAX_SB_PER_CALL; id++)
		{
		  ...
		  if ((INVALID_TUNNEL_ID == tunnel_id3)
		      ||
		      (PPF_SB_PATH_GET_VALID
		       (callline3->sb_multi_path, sb_ids3[id]) == 0))
		    continue;
		  ...
		  if (1 == duplicate)
		  ...
		  else if (duplicate > 1)
		    {
		      ...
		      if (c3)
			{
			  clib_memcpy (c3->opaque2, b3->opaque2,
				       sizeof (b3->opaque2));
			  vnet_buffer2 (c3)->
			    ppf_du_metadata.tunnel_id[VLIB_TX_TUNNEL] =
			    tunnel_id3;
	...
	trace00:
	  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
	      ...
	      if (b0->flags & VLIB_BUFFER_IS_TRACED)
		{
		  ...
		  t->sw_if_index =
		    (tunnel_sw_if_index0 ==
		     ~0) ? sw_if_index0 : tunnel_sw_if_index0;
		  t->next_index = next0;
		}
	    }
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_SB_PATH_LB_ERROR_GOOD, pkts_processed);
...
{
  .function = ppf_sb_path_lb,.name = "ppf_sb_path_lb",.vector_size =
    sizeof (u32),.format_trace = format_ppf_sb_path_lb_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_sb_path_lb_error_strings),.error_strings =
    ppf_sb_path_lb_error_strings,.n_next_nodes =
    PPF_SB_PATH_LB_N_NEXT,.next_nodes =
  {
...
u8 *
format_ppf_lbo_input_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
	       ...
	       vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_LBO_INPUT_ERROR_GOOD, pkts_processed);
...
{
  .function = ppf_lbo_input,.name = "ppf_lbo_input",.vector_size =
    sizeof (u32),.format_trace = format_ppf_lbo_input_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_lbo_input_error_strings),.error_strings =
    ppf_lbo_input_error_strings,.n_next_nodes =
    PPF_LBO_INPUT_N_NEXT,.next_nodes =
  {
