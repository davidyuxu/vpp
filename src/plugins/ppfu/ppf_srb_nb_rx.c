...
u8 *
format_ppf_srb_nb_rx_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ppf_srb_nb_rx_trace_t *t = va_arg (*args, ppf_srb_nb_rx_trace_t *);

  s =
    format (s, "PPF SRB-NB RX: srb outgoing msg received for the call %d \n",
	    clib_net_to_host_u32 (t->srb.call_id));
  s =
    format (s,
	    "  transaction-id %d, request-id %d, sb_num %d, sb_id %d %d %d, length %d",
	    clib_net_to_host_u32 (t->srb.transaction_id),
	    clib_net_to_host_u32 (t->srb.msg.out.request_id),
	    t->srb.msg.out.sb_num, t->srb.msg.out.sb_id[0],
	    t->srb.msg.out.sb_id[1], t->srb.msg.out.sb_id[2],
		      ...
		      vlib_frame_t * from_frame, u32 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  key0 =
	    (uword) PPF_PDCP_COUNT (pdcp0->tx_hfn, pdcp0->tx_next_sn,
				    pdcp0->sn_length);
	  ...
	  if (psm->want_feedback)
	    {
	      msg0.transaction_id =
		clib_net_to_host_u32 (srb0->transaction_id);
	      msg0.request_id =
		clib_net_to_host_u32 (srb0->msg.out.request_id);
	  ...
	  if (PREDICT_TRUE (1 == srb0->msg.out.sb_num))
	    {
	      tunnel_index0 =
		c0->rb.srb.sb_tunnel[srb0->msg.out.sb_id[0]].tunnel_id;
		...
		call_id0;
	      ...
	      vnet_buffer2 (b0)->ppf_du_metadata.path.sb_num =
		srb0->msg.out.sb_num;
	      clib_memcpy (vnet_buffer2 (b0)->ppf_du_metadata.path.sb_id,
			   srb0->msg.out.sb_id, 3);
...
{
  .function = ppf_srb_nb_rx,.name = "ppf_srb_nb_rx",.vector_size =
    sizeof (u32),.format_trace = format_ppf_srb_nb_rx_trace,.type =
    VLIB_NODE_TYPE_INTERNAL,.n_errors =
    ARRAY_LEN (ppf_srb_nb_rx_error_strings),.error_strings =
    ppf_srb_nb_rx_error_strings,.n_next_nodes =
    PPF_SRB_NB_RX_N_NEXT,.next_nodes =
  {
