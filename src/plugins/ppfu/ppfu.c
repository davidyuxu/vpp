...
static clib_error_t *
ppf_gtpu_interface_admin_up_down (vnet_main_t * vnm, u32 hw_if_index,
				  u32 flags)
{
...
void
ppf_reset_callline_intf (u32 call_id)
{
  ...
  if ((call_line->lbo_mode == PPF_LBO_MODE) && (call_line->sw_if_index != ~0))
    {
      vnet_sw_interface_set_flags (vnm, call_line->sw_if_index,
				   0 /* down */ );
      vnet_sw_interface_t *si =
	vnet_get_sw_interface (vnm, call_line->sw_if_index);
      si->flags |= VNET_SW_INTERFACE_FLAG_HIDDEN;

      /* make sure tunnel is removed from l2 bd or xconnect */
      set_int_l2_mode (gtm->vlib_main, vnm, MODE_L3, call_line->sw_if_index,
		       0, 0, 0, 0);
      vec_add1 (gtm->free_ppf_gtpu_tunnel_hw_if_indices,
		call_line->hw_if_index);
...
void
ppf_init_callline_intf (u32 call_id)
{
  ppf_main_t *pm = &ppf_main;
  ppf_gtpu_main_t *gtm = &ppf_gtpu_main;
  u32 hw_if_index = ~0;
...
void
ppf_init_calline (u32 call_id, ppf_calline_type_t call_type)
{
  ppf_main_t *pm = &ppf_main;
...
int
vnet_ppf_add_callline (vnet_ppf_add_callline_args_t * c)
{
				...
				vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	if (unformat
	    (line_input, "call-type %U", unformat_ppf_call_type, &call_type))
	;
	...
	if (unformat
	    (line_input, "mode %U", unformat_ppf_call_mode, &lbo_mode))
	;
  ...
  switch (rv)
    ...
    case 0:
      if (is_add)
	vlib_cli_output (vm,
			 "[Call line added] call-id: %d ue-bear-id: %d, call_type: %d, sb_policy: %d",
			 c.call_id, c.ue_bearer_id, c.call_type,
			 c.ue_bearer_id);
      break;
    ...
    default:
      error = clib_error_return (0, "vnet_ppf_del_callline returned %d", rv);
      goto done;
    }
...
{
.path = "create ppf calline",.short_help =
    "create ppf callline call-id <nn> [ call-type <type-name>] [ue-bear-id <nn>] [sb_policy <nn>]"
    "[mode <tunnel-switch|lbo>] [lbo_vrf_id <nn>] [del]",.function =
    ppf_add_del_calline_command_fn,};
...
ppf_init (vlib_main_t * vm)
{
  ...
  pm->ppf_calline_table =
    clib_mem_alloc_aligned (pm->max_capacity * sizeof (ppf_callline_t),
  ...
  vec_validate_init_empty (pm->buffers_duplicated_per_thread,
...
format_ppf_gtpu_tunnel_simple (u8 * s, va_list * va)
{
  ...
  s =
    format (s, "id %d, type %U", it->tunnel_id, format_ppf_gtpu_tunnel_type,
	    it->tunnel_type);
...
format_ppf_callline (u8 * s, va_list * va)
{
      ...
      if (verbose > 0)
	{
	  ...
	  for (sb = 0; sb < MAX_SB_PER_CALL; sb++)
		...
		format (s, "\nsb tunnel %U\n", format_ppf_gtpu_tunnel_simple,
			&(callline->rb.srb.sb_tunnel[sb]), verbose);
	    }
	}
      else
	{
	  ...
	  for (sb = 0; sb < MAX_SB_PER_CALL; sb++)
	    {
	      s =
		format (s, "\nsb tunnel %U\n", format_ppf_gtpu_tunnel_simple,
			&(callline->rb.drb.sb_tunnel[sb]), verbose);
	    }
	}
      else
	{
  ...
  if (verbose > 0)
    s =
      format (s, "\npdcp %U\n", format_ppf_pdcp_simple, &(callline->pdcp),
	      verbose);
  else
    s =
      format (s, "pdcp {%U}\n", format_ppf_pdcp_simple, &(callline->pdcp),
		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (~0 == call_id)
      ...
      for (call_id = 0; call_id < pm->max_capacity; call_id++)
	{
	  ...
	  if (~0 != callline->call_index)
	    vlib_cli_output (vm, "%U", format_ppf_callline, callline,
			     verbose);
	}
...
{
.path = "show ppf-callline",.short_help =
    "show ppf-callline [index] [verbose]",.function = ppf_show_callline,};
