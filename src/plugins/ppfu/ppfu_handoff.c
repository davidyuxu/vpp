...
format_worker_ppfu_handoff_trace (u8 * s, va_list * args)
{
  ...
  worker_ppfu_handoff_trace_t *t =
    va_arg (*args, worker_ppfu_handoff_trace_t *);

  s =
    format (s,
	    "worker-ppfu_handoff: teid 0x%x, hash 0x%x next_worker %d, buffer 0x%x",
			     ...
			     vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  static __thread vlib_frame_queue_elt_t
    ** ppfu_handoff_queue_elt_by_worker_index;
  static __thread vlib_frame_queue_t
    ** congested_ppfu_handoff_queue_by_worker_index = 0;
  ...
  if (PREDICT_FALSE (ppfu_handoff_queue_elt_by_worker_index == 0))
    {
      vec_validate (ppfu_handoff_queue_elt_by_worker_index,
		    tm->n_vlib_mains - 1);
  ...
  while (n_left_from > 0)
      ...
      if (next_worker_index != current_worker_index)
	{
	  ...
	  fq =
	    is_vlib_frame_queue_congested (hm->frame_queue_index,
					   next_worker_index,
...
format_ppfu_handoff_dispatch_trace (u8 * s, va_list * args)
{
  ...
  ppfu_handoff_dispatch_trace_t *t =
    va_arg (*args, ppfu_handoff_dispatch_trace_t *);
...
format_ppf_pdcp_handoff_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "ppf_pdcp_handoff: call-id 0x%x, hash 0x%x next_worker %d, buffer 0x%x",
			  ...
			  vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  static __thread vlib_frame_queue_t **congested_ppf_pdcp_handoff_queue_by_wi
    = 0;
  ...
  if (PREDICT_FALSE (ppf_pdcp_handoff_queue_elt_by_wi == 0))
    {
      ...
      vec_validate_init_empty (n_left_to_next_pdcp_worker_by_wi,
			       tm->n_vlib_mains - 1, 0);
  ...
  while (n_left_from > 0)
    {
      u32 bi0;
      vlib_buffer_t *b0;
      u32 rx_tunnel_index0 = INVALID_TUNNEL_ID, tx_tunnel_index0 =
	INVALID_TUNNEL_ID;
      ...
      if (PREDICT_FALSE (rx_tunnel_index0 == INVALID_TUNNEL_ID))
	{
	  ...
	  t0 =
	    pool_is_free_index (gtm->tunnels,
				tx_tunnel_index0) ? NULL :
	    pool_elt_at_index (gtm->tunnels, tx_tunnel_index0);
	}
      else
	t0 =
	  pool_is_free_index (gtm->tunnels,
			      rx_tunnel_index0) ? NULL :
	  pool_elt_at_index (gtm->tunnels, rx_tunnel_index0);
      ...
      if (next_worker_index != current_worker_index)
	{
	  ...
	  if (PREDICT_FALSE (0 == hf))
	    {
	      fq =
		is_vlib_frame_queue_congested (hm->pdcp_frame_queue_index,
					       next_worker_index,
					       PPFU_HANDOFF_QUEUE_HI_THRESHOLD,
	      ...
	      if (fq)
		{
	      ...
	      hf =
		vlib_get_worker_handoff_queue_elt (hm->pdcp_frame_queue_index,
						   next_worker_index,
  ...
  if (d)
    {
      vlib_node_increment_counter (vm, node->node_index,
				   PPF_PDCP_HANDOFF_ERROR_BAD, d->n_vectors);
      vlib_put_frame_to_node (vm, hm->error_node_index, d);
    }
  ...
  for (i = 0; i < vec_len (ppf_pdcp_handoff_queue_elt_by_wi); i++)
      ...
      if (ppf_pdcp_handoff_queue_elt_by_wi[i])
	{
	  ...
	  if (1 || hf->n_vectors == hf->last_n_vectors)
	    {
	      hf->n_vectors =
		VLIB_FRAME_SIZE - n_left_to_next_pdcp_worker_by_wi[i];
	      vlib_put_frame_queue_elt (hf);
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_PDCP_HANDOFF_ERROR_GOOD, pkts_processed);
...
format_ppf_pdcp_handoff_dispatch_trace (u8 * s, va_list * args)
{
  ...
  ppf_pdcp_handoff_dispatch_trace_t *t =
    va_arg (*args, ppf_pdcp_handoff_dispatch_trace_t *);
				   ...
				   vlib_frame_t * frame)
{
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_PDCP_HANDOFF_DISPATCH_ERROR_GOOD,
			       pkts_processed);
...
format_ppf_tx_handoff_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "ppf_tx_handoff: call-id 0x%x, hash 0x%x next_worker %d, buffer 0x%x",
	    t->call_id, t->hash, t->next_worker_index, t->buffer_index);
  return s;
}
			...
			vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  static __thread vlib_frame_queue_t **congested_ppf_tx_handoff_queue_by_wi =
    0;
  ...
  if (PREDICT_FALSE (ppf_tx_handoff_queue_elt_by_wi == 0))
    {
      ...
      vec_validate_init_empty (n_left_to_next_tx_worker_by_wi,
			       tm->n_vlib_mains - 1, 0);
  ...
  while (n_left_from > 0)
      ...
      if (next_worker_index != current_worker_index)
	{
	  ...
	  if (PREDICT_FALSE (0 == hf))
	    {
	      hf =
		vlib_get_worker_handoff_queue_elt (hm->tx_frame_queue_index,
						   next_worker_index,
  ...
  for (i = 0; i < vec_len (ppf_tx_handoff_queue_elt_by_wi); i++)
      ...
      if (ppf_tx_handoff_queue_elt_by_wi[i])
	{
	  ...
	  if (1 || hf->n_vectors == hf->last_n_vectors)
	    {
	      hf->n_vectors =
		VLIB_FRAME_SIZE - n_left_to_next_tx_worker_by_wi[i];
	      vlib_put_frame_queue_elt (hf);
...
format_ppf_tx_handoff_dispatch_trace (u8 * s, va_list * args)
{
  ...
  ppf_tx_handoff_dispatch_trace_t *t =
    va_arg (*args, ppf_tx_handoff_dispatch_trace_t *);
				 ...
				 vlib_frame_t * frame)
{
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       PPF_TX_HANDOFF_DISPATCH_ERROR_GOOD,
			       pkts_processed);
...
ppfu_handoff_init (vlib_main_t * vm)
{
  ...
  if (hm->num_workers < 2)
  ...
  else
    {
      ...
      hm->first_pdcp_worker_index =
	hm->first_tx_worker_index + hm->num_tx_workers;

      if ((ppf_main.io_mode == PPF_IO_MODE_DEDICATED)
	  && (0 == (hm->num_tx_workers % 2)))
	{
	  hm->num_tx_workers >>= 1;
	  hm->first_tx_worker_index += hm->num_tx_workers;
	}
			     ...
			     vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat_user
	  (input, unformat_vlib_enable_disable, &handoff_enable))
	;
	...
	if (unformat
	    (input, "trace %U", unformat_vlib_enable_disable, &trace))
	;
      else
	{
	  error =
  ...
  if (trace != ~0)
    {
      ...
      if (trace)
	{
	  vec_validate_init_empty_aligned (hm->pdcp_trace_index,
					   hm->num_workers, 0,
	  ...
	  vec_validate_init_empty_aligned (hm->tx_trace_index,
					   hm->num_workers, 0,
					   CLIB_CACHE_LINE_BYTES);
  ...
  vlib_cli_output (vm, "PPF handoff %s, io-mode %s\n",
		   ...
		   ((ppf_main.io_mode ==
		     PPF_IO_MODE_DEDICATED) ? "dedicated" : "shared"));
