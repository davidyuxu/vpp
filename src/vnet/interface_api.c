...
vl_api_get_sw_interface_stats_t_handler (vl_api_get_sw_interface_stats_t * mp)
{
  ...
  vl_api_sw_interface_counters_t counter[SW_INTERFACE_COUNTER_BATCH_SIZE] =
    { 0 };
  ...
  count =
    count <
    SW_INTERFACE_COUNTER_BATCH_SIZE ? count : SW_INTERFACE_COUNTER_BATCH_SIZE;
  ...
  for (i = 0; i < count; i++)
      ...
      if (vec_len (ccm) > counter[i].sw_if_index)
	{
	  ...
	  clib_mem_unaligned (&counter[i].rx_pkts, u64) =
	    clib_host_to_net_u64 (v.packets);
	  clib_mem_unaligned (&counter[i].rx_bytes, u64) =
	...
	{
	  vlib_get_combined_counter (ccm, counter[i].sw_if_index, &v);
	  clib_mem_unaligned (&counter[i].tx_pkts, u64) =
	    clib_host_to_net_u64 (v.packets);
	  clib_mem_unaligned (&counter[i].tx_bytes, u64) =
	    clib_host_to_net_u64 (v.bytes);
	}
      ...
      if (vec_len (scm) > counter[i].sw_if_index)
	{
	  ...
	  clib_mem_unaligned (&counter[i].drop, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].punt, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].ip4, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].ip6, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].rx_no_buf, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].rx_miss, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].rx_error, u64) =
	    clib_host_to_net_u64 (c);
	}
	...
	{
	  c = vlib_get_simple_counter (scm, counter[i].sw_if_index);
	  clib_mem_unaligned (&counter[i].tx_error, u64) =
	    clib_host_to_net_u64 (c);
	}
  ...
  REPLY_MACRO4 (VL_API_GET_SW_INTERFACE_STATS_REPLY, mp_size,
		...
		memcpy (rmp->data, counter,
			count * sizeof (vl_api_sw_interface_counters_t));
		}
  );
}
