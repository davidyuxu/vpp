  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "del %u", &sw_if_index))
	{
	  if (throughput_stats_main.throughput_debug == 1)
	    {
	      vlib_cli_output (vm,
			       "Please disable throughput trace before delete a interface trace.\n");
	    }
	  else
	    {
...
throughput_stats_lock ()
{
  if (throughput_stats_main.if_counter_lock)
    while (__sync_lock_test_and_set
	   (throughput_stats_main.if_counter_lock, 1))
      /* zzzz */ ;
}
...
throughput_stats_init (stats_main_t * sm)
{
  ...
  throughput_stats_main.if_counter_lock =
...
throughput_stats_counter_show_ (vlib_if_stats_main_t * sm)
{
  ...
  vec_foreach_index (i, sm->counters)
  {
	...
	vlib_cli_output (vm, "%10d %10d %15llu %15llu %20llu %20llu\n",
			 sm->sw_if_index, i, c->rx.packets, c->tx.packets,
			 c->rx.bytes, c->tx.bytes);
	show_title = 0;
      }
    else
      {
	vlib_cli_output (vm, "           %10d %15llu %15llu %20llu %20llu\n",
			 i, c->rx.packets, c->tx.packets, c->rx.bytes,
			 c->tx.bytes);
      }
  ...
  vlib_cli_output (vm, "           %10s %15llu %15llu %20llu %20llu\n",
		   "Total", ct.rx.packets, ct.tx.packets, ct.rx.bytes,
		   ct.tx.bytes);
  ...
  if (verbose == 0)
    {
      ...
      vlib_cli_output (vm, "\n%10s %10s %15s %15s %20s %20s\n", "Interface",
		       "Thread", "RxPkts", "TxPkts", "RxBytes", "TxBytes");
...
throughput_over_interval_get (u32 interval, u32 * mbps, u32 * pps)
{
  ...
  if (throughput_slot > (number_of_bins - 1))
    ...
    {
      starting_bin =
	SYSTEM_THROUGHPUT_MAX_BINS - number_of_bins + throughput_slot;
  ...
  for (i = starting_bin; i < ending_bin; i++)
    {
      vec_foreach (sm,
		   throughput_stats_main.if_stats_by_bin[i %
							 SYSTEM_THROUGHPUT_MAX_BINS])
      {
...
throughput_show ()
{
  ...
  slot =
    (throughput_slot ==
     0) ? (SYSTEM_THROUGHPUT_MAX_BINS - 1) : (throughput_slot - 1);

  vlib_cli_output (vm, "\n%10s %10s %10s %10s %15s %15s\n", "Interface",
		   "Thread", "Rx", "Tx", "Rx_Mbps", "Tx_Mbps");
  ...
  vec_foreach (sm, throughput_stats_main.if_stats_by_bin[slot])
  {
      ...
      if (show_if_index == 1)
	{
	  vlib_cli_output (vm, "%10d %10d %10llu %10llu %15llu %15llu\n",
			   ...
			   c->rx.bytes / SYSTEM_THROUGHPUT_TIMER_INTERVEL /
			   ONE_MB * 8,
			   c->tx.bytes / SYSTEM_THROUGHPUT_TIMER_INTERVEL /
			   ONE_MB * 8);
	  show_if_index = 0;
	}
      else
    ...
    vlib_cli_output (vm, "           %10s %10llu %10llu %15llu %15llu\n",
		     ...
		     ct.rx.bytes / SYSTEM_THROUGHPUT_TIMER_INTERVEL / ONE_MB *
		     8,
		     ct.tx.bytes / SYSTEM_THROUGHPUT_TIMER_INTERVEL / ONE_MB *
		     8);
...
do_throughtput_measure (stats_main_t * sm)
{
  ...
  vec_foreach_index (i, throughput_stats_main.if_stats)
  {
    ...
    bin_ism =
      vec_elt_at_index (throughput_stats_main.if_stats_by_bin
			[throughput_slot], i);
    ...
    vec_foreach_index (n_thread, last_ism->counters)
    {
      ...
      rx_c =
	&im->
	combined_sw_if_counters[VNET_INTERFACE_COUNTER_RX].counters[n_thread]
	[sw_if_index];
      tx_c =
	&im->
	combined_sw_if_counters[VNET_INTERFACE_COUNTER_TX].counters[n_thread]
	[sw_if_index];
  ...
  throughput_over_interval_get
    (throughput_stats_main.throughput_threshold_interval, &mbps_interval,
     &pps_interval);
  ...
  if (throughput_stats_main.throughput_debug)
    {
      vlib_cli_output (vm,
		       ...
		       SYSTEM_THROUGHPUT_TIMER_INTERVEL, mbps, pps,
		       throughput_stats_main.throughput_threshold_interval,
		       mbps_interval, pps_interval);
